...
//                 pthread_mutex_lock(&socket_mutex);
//                 ssize_t sent = send(sock, result, 2, MSG_NOSIGNAL);  // write 대신 send 사용
//                 if (sent <= 0) {
//                     printf("%s결과 전송 실패 (error: %s)\n", thread_log, strerror(errno));
//                     consecutive_errors++;
//                 } else {
//                     printf("%s거리: %.2fcm, %s 감지됨, 결과: %s 전송됨\n", 
//                            thread_log, distance, detected ? "물체" : "물체 미", result);
...
//                     printf("%s연속 전송 실패, 쓰레드 종료\n", thread_log);
//                     return NULL;
//                 }
//             }
//             usleep(100000);  // 100ms 간격으로 측정
//         }
//         usleep(10000);
//     }
//     return NULL;
...
//         str_len = read(sock, recv_msg, 2);  // 2바이트만 읽기
